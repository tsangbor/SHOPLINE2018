...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) {
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			//$classes = array( 'dropdown-menu' ); #fix 20180313
			...
			if ( end( $matches[2] ) ) {
				...
				$listidby = 'id="' . end( $matches[2] ) . '"';
			}
			$output .= "{$n}{$indent}<ul$class_names $labelledby $listidby role=\"menu\" >{$n}";
		}
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( $args->has_children && 0 === $depth && $args->depth > 1 ) {
				$atts['href']          = '#menu-item-dropdown-' . $item->ID;
				$atts['data-toggle']   = 'collapse';
				$atts['aria-haspopup'] = 'true';
				$atts['aria-expanded'] = 'false';
				$atts['class']         = 'dropdown-toggle btn--collapse nav-link';
				$atts['data-id']            = 'menu-item-dropdown-' . $item->ID;
			} else {
				$atts['href'] = ! empty( $item->url ) ? $item->url : '#';
				// Items in dropdowns use .dropdown-item instead of .nav-link.
				if ( $depth > 0 ) {
					//$atts['class'] = 'dropdown-item'; #20180313
					$atts['class'] = 'nav-link';
				} else {
					$atts['class'] = 'nav-link';
				}
